var PluginDetect = { version: "0.7.8", name: "PluginDetect", handler: function (c, b, a) {
    return function () {
        c(b, a)
    }
...
a++) {
    ...
    if (a > 0 && b.isFunc(c[0])) { } else { if (b.isFunc(c)) { } } 
    ...
    if (d.div && d.div.childNodes) {
        for (b = d.div.childNodes.length - 1;
b >= 0;
b--) {
            c = d.div.childNodes[b];
            if (c && c.childNodes) {
                for (h = c.childNodes.length - 1;
h >= 0;
h--) {
                    g = c.childNodes[h];
                    try {
...
a = a + 2) {
    ...
    if (!g) {
        try {
            j.write(c + 'div id="' + b + '">o' + c + "/div>", "gl", "gl");
...
o = o + 2) {
        ...
        try {
            p.innerHTML = parseHTML(parseHTML(q))
        } catch (m) { };
        try {
            k.div.appendChild(p)
        } catch (m) { };
...
}, Plugins: { java: { mimeType: ["application/x-java-applet", "application/x-java-vm", "application/x-java-bean"], classID: "clsid:8AD9C840-044E-11D1-B3E9-00805F499D93", navigator: { a: window.navigator.javaEnabled(), javaEnabled: function () {
    return this.a
...
}, DTK: { $: 1, hasRun: 0, status: null, VERSIONS: [], version: "", HTML: null, Plugin2Status: null, classID: ["clsid:CAFEEFAC-DEC7-0000-0001-ABCDEFFEDCBA", "clsid:CAFEEFAC-DEC7-0000-0000-ABCDEFFEDCBA"], mimeType: ["application/java-deployment-toolkit", "application/npruntime-scriptable-plugin;DeploymentToolkit"], disabled: function () {
...
}, JavaVersions: [[1, 9, 1, 40], [1, 8, 1, 40], [1, 7, 1, 40], [1, 6, 0, 40], [1, 5, 0, 30], [1, 4, 2, 30], [1, 3, 1, 30]], query: function () {
...
l++) {
    ...
    var m = this, e = m.$, c = m.$$, h = c.navigator, j, l, k, g, d, a, i, f = 0, b = (m.hasRun || m.disabled());
    m.hasRun = 1;
    if (b) {
        return m
    };
    ...
    if (!f || c.debug) {
        ...
        if ((g.test(a) || g.test(i)) && parseInt(RegExp.$1, 10) >= 5) { f = "1," + RegExp.$1 + "," + (RegExp.$2 ? RegExp.$2 : "0") + "," + (RegExp.$3 ? RegExp.$3 : "0") } 
    } if (!f || c.debug) {
        g = /Java[^\d]*Plug-in/i;
    ...
    if (!b) {
        ...
        } catch (i) { } if (g[a] && g.isArray(g[a])) {
            f.result = [].concat(g[a])
        } 
    } return f.result
} 
}
}, applet: { $: 1, results: [[null, null], [null, null], [null, null]], getResult: function () {
